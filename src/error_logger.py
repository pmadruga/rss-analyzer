...
class ErrorLogger:
    ...
    def _update_processing_status(self, date: str, success: bool):
        ...
        try:
            ...
            daily_status = status_data.get('dates', {}).get(date, {
                'date': date,
                'status': 'unknown',
                'total_attempted': 0,
                'successful': 0,
            ...
            daily_status['last_attempt'] = datetime.now(timezone.utc).isoformat()
    ...
    def get_status_summary(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            if not self.status_file.exists():
                return {'dates': {}, 'summary': {'total_days': 0, 'successful_days': 0, 'failed_days': 0}}
            ...
            successful_days = sum(1 for s in recent_dates.values() if s.get('status') == 'success')
            failed_days = sum(1 for s in recent_dates.values() if s.get('status') == 'failed')
    ...
    def get_recent_errors(self, days: int = 7) -> List[ProcessingError]:
        ...
        try:
            ...
            for error_dict in errors:
                try:
                    error_date = datetime.fromisoformat(error_dict['timestamp'].replace('Z', '+00:00'))
...
def log_api_error(error_type: str, error_message: str, api_provider: str, details: Optional[Dict[str, Any]] = None):
...
def log_scraping_error(error_type: str, error_message: str, url: str, details: Optional[Dict[str, Any]] = None):
...
def log_processing_success(component: str, message: str, details: Optional[Dict[str, Any]] = None):
