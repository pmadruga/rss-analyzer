...
class ArticleProcessor:
    ...
    def __init__(self, config: dict):
        ...
        self.rss_parser = RSSParser(
            config.get("user_agent", "RSS-Article-Analyzer/1.0")
        )
    ...
    def run(self, force_refresh: bool = False, limit: int | None = None) -> dict:
        ...
        try:
            ...
            if not force_refresh:
                ...
                logger.info(
                    f"Skipping {len(rss_entries) - len(new_entries)} already analyzed articles"
                )
            else:
            ...
            for entry in entries:
                try:
                    ...
                    self.db.log_processing(
                        article_id, "started", processing_step="scraping"
                    )
                    ...
                    self.db.insert_content(
                        article_id, scraped_content.content, analysis
                    )
...
def test_api(ctx):
    ...
    try:
        ...
        if api_provider == "mistral":
            ...
            click.echo(
                f"❌ Unsupported API provider: {api_provider}. Supported: mistral, openai",
                err=True,
            )
            sys.exit(1)
    ...
    try:
        ...
        if detailed:
            ...
            for provider in ["anthropic", "mistral", "openai"]:
                ...
                click.echo(
                    f"  Max Retries: {provider_config.get('max_retries', 'Not configured')}"
                )
                click.echo(
                    f"  Timeout: {provider_config.get('timeout', 'Not configured')}s"
                )
...
@click.option(
    "--output",
    "-o",
    default="logs/api_health_report.json",
