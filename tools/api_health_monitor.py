...
class APIHealthMonitor:
        ...
        try:
            ...
            if not api_key:
                return APITestResult(
                    ...
                    http_status=None,
                    model_used=self.config.get("api", {})
                    .get("anthropic", {})
                    .get("model"),
                    credits_info=None,
    ...
    async def run_health_check(self) -> MonitoringReport:
        ...
        self.logger.info(
            f"✅ Health check complete: {working_apis}/{len(api_results)} APIs working"
        )
        return report

    def save_report(
        self, report: MonitoringReport, output_file: str = "logs/api_health_report.json"
    ):
